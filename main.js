...
const PATH='orcamento365_9b8e04c5';
   // garante auth.uid antes dos gets/sets
const save=(k,v)=>localStorage.setItem(k, JSON.stringify(v));const load=async(k,d)=>{const s=localStorage.getItem(k);return s ? JSON.parse(s) : d;};
...
document.getElementById("toggleCards").onclick = () => {
  document.getElementById("cardPanel").toggleAttribute("open");
};
