...
export function getReservedTotalForDate(dateISO, transactions) {
  ...
  // Collect all recurring masters to identify which budgets are from materialized transações
  ...
  let total = budgets.reduce((acc, budget) => {
    ...
    // SKIP recurring budgets that come from materialized transactions
    // (they're already counted in daily balances via txByDate materialization)
  ...
  // so they're already counted as real transactions. Including them again would double-count.
